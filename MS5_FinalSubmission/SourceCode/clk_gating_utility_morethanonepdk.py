...
def utility():
    ...
    if len(sys.argv)==3:
        pdk = sys.argv[2]
        if str(pdk)== "default" or str(pdk)== "sky130":
            mux = "sky130_fd_sc_hd__mux2_1"
            ff="sky130_fd_sc_hd__dfxtp_1"
            icg="sky130_fd_sc_hd__dlclkp"
        else:
            mux1 = pdk+"_mux2_1"
            ff1= pdk+"_dfxtp_1"
            icg1 = pdk+"_dlclkp"
    ...
    #***** MAIN LOGIC *********
    parsingprocess(definition=definition,mux=mux,ff=ff,icg=icg,mux1=mux1,ff1=ff1, icg1=icg1,anotherpdk= anotherpdk)
    #***** GENERATING OUTPUT ************
    generateOutFile(ast=ast)

#**********************************************
def ff_in_list(definition,ff):
    ff_l=[]
    ff_counter=0
...
def mux_ff_pattern(definition,pattern_found,instance,ffin_list,muxout):
    ...
    return pattern_found
#*************************************
def morethan_1_reg_logic(definition,Enables_num,Enables_total,ICG_count_list,ffin_list,mux,ff): 
    ICG_count_list_noduplicate= []
    mux_outs = []
    ff_inputs = []
    unique_en = ""
    unique_mux = ""
    ff_inputs =ff_in_list(definition=definition,ff=ff)
    for itemDeclaration in definition.items:
        item_type = type(itemDeclaration).__name__
        if item_type == "InstanceList":
            instance = itemDeclaration.instances[0]
            if instance.module == mux: ##check for all the muxes
...
def check_size(ffin_list,Enables_num):
    ...
    return size
#************************************************
def icg_cells (Enables_num, Enables_total,icg_count,newitems,ICG_count_list,ffin_list,icg,icg1,anotherpdk):
    ## SIZES CHECK ##
   ...
   # print(Enables_num)
    ...
    while icg_count < Enables_num:
        ...
        if not anotherpdk:
            ...
            clock_gate_cell = vast.Instance(icg,"__clock_gate_cell__"+str(i),tuple(clock_gate_args),tuple()) 
            newitems.append(vast.InstanceList(icg+"_"+str(icg_size), tuple(), tuple([clock_gate_cell])))
            i+=1
            ...
            clock_gate_cell = vast.Instance(icg1,"__clock_gate_cell__"+str(i),tuple(clock_gate_args),tuple()) 
            newitems.append(vast.InstanceList(icg1+"_"+str(icg_size), tuple(), tuple([clock_gate_cell])))
            i+=1
            icg_count = icg_count+1

    return
#***********************************************
def handle_ff(definition, Enables_num, L3,ffin_list,ff,icg):
    icg_outputs = []
    right_order =[]
    i= 0
    ii =0
...
#*********************************************
def parsingprocess(definition,mux,ff,icg,mux1,ff1,icg1,anotherpdk):
    newitems = []
    WithoutMux = True
    icg_instances = False
    ...
    ffin_list = ff_in_list(definition=definition,ff=ff)   # return list with all the inputs of all the flip flops   
    ...
    ,ffin_list=ffin_list,mux=mux,ff=ff)         
...
#************ MAIN logic ********
    for itemDeclaration in definition.items:
        ...
        if item_type == "InstanceList":
            ...
            if not icg_instances:
                icg_cells(Enables_num=Enables_num, Enables_total = Enables_total, icg_count=icg_count, newitems=newitems,ICG_count_list=ICG_count_list,ffin_list=ffin_list,icg=icg,icg1=icg1,anotherpdk=anotherpdk) ##function that adds icg cells
                icg_instances=True

            if instance.module == mux:
                for hook in instance.portlist:
                    if hook.portname == "X":
                        muxout = hook.argname
            ...
            for hook in instance.portlist:
                ...
                while (ll <len(ffin_list)):

                    customized_ff_args = [
                    vast.PortArg("CLK", vast.Identifier("cg_out")),
                    vast.PortArg("D", vast.Identifier("d_in")),
                    vast.PortArg("Q", vast.Identifier("d_out"))]

                    customized_ff = vast.Instance(ff1,"__Flip_Flop__",tuple(customized_ff_args),tuple()) 
                    newitems.append(vast.InstanceList(ff1, tuple(), tuple([customized_ff])))
                    ll+=1
    ...
    handle_ff(definition=definition, Enables_num=Enables_num,L3=L3,ffin_list=ffin_list,ff=ff,icg=icg)
